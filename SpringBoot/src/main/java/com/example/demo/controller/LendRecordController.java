...
import com.example.demo.entity.RequestLendRecord;
...
public class LendRecordController {
    ...
    @PutMapping("/{isbn}")
    public Result<?> update(@PathVariable String isbn, @RequestBody RequestLendRecord requestLendRecord) {
        UpdateWrapper<LendRecord> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("isbn", isbn);
        LendRecord lendrecord = new LendRecord();
        lendrecord.setLendTime(requestLendRecord.getLendTime());
        lendrecord.setReturnTime(requestLendRecord.getReturnTime());
        lendrecord.setStatus(requestLendRecord.getStatus());
        LendRecordMapper.update(lendrecord, updateWrapper);
        return Result.success();
    }

    @PutMapping("/time/{lendTime}")
    public Result<?> update2(@PathVariable Date lendTime, @RequestBody RequestLendRecord requestLendRecord) {
        UpdateWrapper<LendRecord> updateWrapper = new UpdateWrapper<>();
